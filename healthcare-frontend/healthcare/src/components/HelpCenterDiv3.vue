<template>
  <div class="mt-10 px-10 mx-10">
    <h2 class="hd1 genQ">General Questions</h2>
    <v-expansion-panels multiple>
      <v-row class="mt-5">
        <v-col cols="12" md="6" sm="12">
          <v-expansion-panel
             v-for="item in faqInfo" v-bind:key="item.id"
            class="panels mb-5"
            style="border: 0px"
          >
...
export default {
  data() {
    return {
      // genQue: [
      //   {
      //     que: "What are the signs and symptoms of step throat",
  ...
  methods:{
    async getFaqInfo() {
     let response = await axios.get('http://localhost:9000/api/faq/get')
...
/*.genQ::before{
  ...
  width: 100%;
  height: 2px;
  background-color:rgb(226, 59, 100) ;
  transform: scaleX(.1);
